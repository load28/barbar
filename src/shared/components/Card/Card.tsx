...
export const CardHeader = forwardRef<HTMLDivElement, CardHeaderProps>((props, ref) => {
  const { title, subtitle, action, icon, children, className, ...rest } = props;
    ...
    <div ref={ref} className={combinedClassName} {...rest}>
      <div className={cardHeaderContainer}>
        <div className={cardHeaderContent}>
          {icon && <div className={cardHeaderIconContainer}>{icon}</div>}
          <div>
            {title && <h3 className={cardTitle}>{title}</h3>}
            {subtitle && <div className={cardSubtitle}>{subtitle}</div>}
          </div>
        </div>
        {action && <div className={cardHeaderActionContainer}>{action}</div>}
      </div>
      {children}
    </div>
  );
...
export const CardContent = forwardRef<HTMLDivElement, CardContentProps>((props, ref) => {
  const { children, noPadding = false, className, ...rest } = props;
...
export const CardImage = forwardRef<HTMLImageElement, CardImageProps>((props, ref) => {
  const { alt, className, ...rest } = props;

  // 클래스 이름 결합
  const combinedClassName = `${cardImage} ${className || ''}`;

  return <img ref={ref} alt={alt} className={combinedClassName} {...rest} />;
});
